    ...
    try {
      if (name && email && password) {
        ...
        const token = jwt.sign(
          { email: email }, 
          process.env.JWT_SECRET, // Fix: Remove quotes around process.env
          { expiresIn: "10m" }
        );
        ...
        await sendEmailtoUser(link, email, name, password);
  ...
  static forgetPassword = async (req, res) => {
    ...
    try {
      if (email) {
        ...
        if (isUser) {
          // generate token
          const secretKey = isUser._id + "process.env.JWT_SECRET";
          const token = jwt.sign({ userID: isUser._id }, secretKey, {
            expiresIn: "5m",
          });
  
          const link = `http://13.232.78.236:3000/user/reset/${isUser._id}/${token}`;
    ...
    try {
      if (email) {
        ...
        if (isUser) {
          ...
          const link = `http://13.232.78.236:9000/user/sendEmail/${isUser._id}/${token}`;
