...
const Instructor_Courses = ({ videoPreview: propVideoPreview }) => {
  ...
  const fetchVideos = async () => {
    try {
      const response = await fetch('http://13.232.0.0:3000/api/videos');
      if (response.ok) {
  ...
  const handleUpload = async () => {
    ...
    try {
      const response = await fetch('http://13.232.0.0:3000/api/upload', {
        method: 'POST',
        body: formData,
      });
  ...
  const handleDelete = async (videoId) => {
    ...
    try {
      const response = await fetch(`http://13.232.0.0:3000/api/videos/${videoId}`, {
        method: 'DELETE',
      });
