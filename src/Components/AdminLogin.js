...
const AdminLogin = () => {
  ...
  const sendOtp = async () => {
    ...
    try {
      const response = await axios.post('http://15.207.19.75:9000/send-otp', { email });
  ...
  const verifyOtp = async () => {
    try {
      const response = await axios.post('http://15.207.19.75:9000/verify-otp', { email, userOtp: otp });
      setMessage(response.data.message);
