...
const Sign_UP = () => {
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      if (user) {
        ...
        const response = await axios.post("http://13.232.78.236:9000/api/users/save", {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      const response = await axios.post("http://13.232.78.236:9000/api/auth/users/register", input);
