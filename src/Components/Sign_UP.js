...
const Sign_UP = () => {
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      if (user) {
        ...
        const response = await axios.post("http://15.207.19.75:9000/api/users/save", {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      const response = await axios.post("http://15.207.19.75:9000/api/auth/users/register", input);
